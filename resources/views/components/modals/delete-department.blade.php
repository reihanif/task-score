<div class="table-row-button cursor-default">
    ...
    <div class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
        id="delete-departments-modal-{{ $id }}"
        tabindex="-1">
        <form x-on:submit="loading = ! loading" action="{{ route('departments.destroy', $id) }}"
            method="post">
            @csrf
            @method('delete')
            <div class="relative max-h-full w-full max-w-md p-4">
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    <button class="absolute end-2.5 top-3 ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white"
                        data-modal-hide="delete-departments-modal-{{ $id }}"
                        type="button">
