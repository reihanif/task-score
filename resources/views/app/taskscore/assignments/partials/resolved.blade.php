        ...
        <div>
            <div class="relative">
                ...
                <input class="block w-auto rounded-lg border border-gray-300 bg-gray-50 ps-10 pt-2 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
                       data-filter-target="resolved-assignments-table"
            ...
            <tbody>
                @foreach ($resolved_assignments as $key => $item)
                    <tr class="cursor-pointer border-b bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-600"
                        ...
                        <td class="whitespace-nowrap px-3 py-4"
                            data-order="{{ $item->resolved_at->format('YmdHi') }}">
                            {{ $item->resolved_at->format('d M Y H:i') }}
                        </td>
                        <td class="whitespace-nowrap px-3 py-4">
                            @if ($item->score() == 100)
                            ...
                            @elseif ($item->score() > 100)
                                <div class="align-items-center flex gap-1 text-green-600 dark:text-green-500">
                                    {{ $item->score() . '%' }}
                                    <span>
                                        <svg class="inline-block h-4 w-4"
                                             xmlns="http://www.w3.org/2000/svg"
                            ...
                            @elseif ($item->score() < 100)
                                <div class="align-items-center flex gap-1 text-red-600 dark:text-red-500">
                                    {{ $item->score() . '%' }}
                                    <span>
                                        <svg class="inline-block h-4 w-4"
                                             xmlns="http://www.w3.org/2000/svg"
