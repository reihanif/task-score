<div class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
    id="unset-recurring-modal-{{ $assignment->id }}"
    tabindex="-1">
    <form x-on:submit="loading = ! loading"
        action="{{ route('taskscore.assignment.recurrence.delete', ['assignment' => $assignment->id, 'recurrence' => $assignment->recurrence->id]) }}"
        method="post">
        @csrf
        @method('delete')
        <div class="relative max-h-full w-full max-w-md p-4">
            <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                <button
                    ...
                    data-modal-hide="unset-recurring-modal-{{ $assignment->id }}"
                    type="button">
                ...
                <div class="p-4 text-center md:p-5">
                    ...
                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Are you sure wan't to remove the recurring schedule for this assignment?</h3>
